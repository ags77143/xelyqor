...
import { BookOpen, FileText, Calendar, Zap, ChevronRight, ArrowLeft, Clock, Upload } from "lucide-react";
...
export default function SubjectPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const lecs = await apiGet(`/lectures/?user_id=${session.user.id}&subject_id=${id}`);
      setLectures(lecs);
      setLoading(false);
    });
  }, [id]);
  ...
  const generateStudyPlan = async () => {
    ...
    try {
      ...
      toast.error("Failed to generate study plan: " + e.message);
    } finally {
  ...
  const generatePracticeExam = async () => {
    ...
    try {
      ...
      toast.error("Failed to generate exam: " + e.message);
    } finally {
    ...
    <div className="min-h-screen bg-cream">
      <div className="max-w-4xl mx-auto px-6 py-8">
        <button
          ...
          className="flex items-center gap-2 text-ink-light hover:text-ink text-sm mb-6 transition-colors"
        >
          <ArrowLeft size={16} />
          Back to Library
        </button>
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-12 h-12 rounded-2xl flex-shrink-0" style={{ backgroundColor: subject?.colour || "#c17b2e" }} />
          <div>
            <h1 className="font-serif text-3xl text-ink">{subject?.name}</h1>
        ...
        {showExamUpload && (
          <div className="bg-white border border-cream-darker rounded-2xl p-5 mb-6">
            <h3 className="font-semibold text-ink text-sm mb-3">Generate Practice Exam</h3>
            <p className="text-xs text-ink-light mb-4">Optionally upload a past exam paper (PDF) to tailor the style and format to your uni.</p>
            <label className="flex items-center gap-2 cursor-pointer text-sm text-ink-light hover:text-ink transition-colors mb-4">
              ...
              <input
                ...
                accept=".pdf"
                className="hidden"
                onChange={(e) => setPastPaper(e.target.files[0])}
              />
            </label>
            <button
        ...
        {/* Exam date input */}
        {showExamInput && (
          <div className="bg-white border border-cream-darker rounded-2xl p-5 mb-6 flex items-center gap-4">
        ...
        {/* Tabs */}
        <div className="flex gap-1 border-b border-cream-darker mb-6">
          {["lectures", "summary", "exam", "studyplan"].map((t) => (
            <button
              key={t}
              onClick={() => setActiveTab(t)}
              className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${
                activeTab === t ? "border-amber text-amber" : "border-transparent text-ink-light hover:text-ink"
              }`}
            >
          ...
          <div className="space-y-3">
            {lectures.length === 0 ? (
              ...
              lectures.map((l) => (
                <div
                  key={l.id}
                  onClick={() => window.location.href = `/?lecture=${l.id}`}
          ...
          <div>
            {!summary ? (
              <div className="text-center py-16">
                ...
                <p className="text-ink-light text-sm mb-6">AI will analyse all {lectures.length} lectures and create a master checklist of everything you need to know.</p>
                <button
                  onClick={generateSummary}
                  disabled={generatingSummary}
          ...
          <div>
            {!practiceExam ? (
              <div className="text-center py-16">
                ...
                <p className="text-ink-light text-sm mb-6">Click Practice Exam above to generate a full exam paper. Upload a past paper to match your uni's style.</p>
              </div>
            ) : (
              <div className="space-y-6">
                {/* Exam header */}
                ...
                {practiceExam.sections?.map((section, si) => (
                  <div key={si} className="bg-white border border-cream-darker rounded-2xl overflow-hidden">
                    ...
                    {section.instructions && (
                      <p className="px-6 py-2 text-xs text-ink-light italic border-b border-cream-darker">{section.instructions}</p>
                    )}
                        ...
                        <div key={qi} className="p-6">
                          <div className="flex items-start justify-between gap-4 mb-3">
                            <p className="text-ink text-sm font-medium leading-relaxed flex-1">
                              <span className="font-bold text-amber mr-2">Q{q.number}.</span>
                              {q.question}
                            </p>
                              ...
                              {q.options.map((opt, oi) => (
                                <button
                                  ...
                                  className={`w-full text-left px-4 py-2 rounded-lg text-sm transition-colors ${
                                    examAnswers[`${si}-${qi}`] === oi
                                      ? "bg-amber-pale border-2 border-amber text-amber font-medium"
                                      : "bg-cream border border-cream-darker text-ink hover:bg-cream-darker"
                                  }`}
                                >
                                  {opt}
                                </button>
          ...
          <div>
            {!studyPlan ? (
              <div className="text-center py-16">
                ...
                <p className="text-ink-light text-sm mb-6">Click the Study Plan button above and enter your exam date.</p>
              </div>
            ) : (
              <div className="space-y-6">
                <div className="bg-white border border-cream-darker rounded-2xl p-6">
...
function ChecklistItem({ text }) {
  ...
  return (
    <div
      ...
      className={`flex items-start gap-3 p-3 rounded-xl cursor-pointer transition-colors ${checked ? "bg-green-50" : "hover:bg-cream"}`}
    >
